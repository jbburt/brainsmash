.. BrainSMASH documentation master file, created by
   sphinx-quickstart on Mon Jan 13 10:41:07 2020.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

BrainSMASH
**********

BrainSMASH (Brain Surrogate Maps with Autocorrelated Spatial Heterogeneity) is a
python-based computational platform for statistical testing of spatially
autocorrelated brain maps. At the core of BrainSMASH is functionality to
simulate surrogate brain maps with spatial autocorrelation that is matched
to spatial autocorrelation in a target brain map. Additional utilities are provided
for Connectome Workbench users who wish to analyze surface-based neuroimaging files.

Contents:
=========

.. toctree::
   :maxdepth: 1

   installation
   approach
   gettingstarted
   example
   api/index_api
   references

.. tutorials

Authors
=======
- Joshua B. Burt, Yale University
- John D. Murray, Yale University

License
=======
The BrainSMASH source code is available under the GNU General Public License v3.0.

Support
=======
If you run into a problem or identify a bug in the source code, please send an email to joshua.burt@yale.edu or `create a new issue on GitHub <https://github.com/murraylab/brainsmash/issues>`_.

.. Indices and tables
   ==================
   * :ref:`genindex`
   * :ref:`modindex`
   * :ref:`search`
